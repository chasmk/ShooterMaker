...
设置E键按下时，若面前有武器（通过显示拾取提示来判断），则交换；若没有，则扔出当前武器。

## 10 换弹 Reload

按下R键换弹。主要看两个值：①武器类中当前的Ammo数，②角色类中的后备弹药数。

- 若该子弹类型的后备弹药为空/当前弹夹是满的，则不换弹

- 根据上面两个值计算出可补充的子弹数。然后更新变量的值。
- Widget中bind相关变量，在ui中同步显示即可。
