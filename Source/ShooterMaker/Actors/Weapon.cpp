...
int32 AWeapon::DecrementAmmo()
{
	return Ammo = FMath::Clamp(Ammo - 1, 0, Ammo);
}

void AWeapon::IncrementAmmo(int32 DeltaValue)
{
	Ammo = FMath::Clamp(Ammo + DeltaValue, 0, AmmoCapacity);
}

USkeletalMeshSocket const* AWeapon::GetBarrelSocket()
{
	return GetItemMesh()->GetSocketByName(BarrelSocketName);
}
